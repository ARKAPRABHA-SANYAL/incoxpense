...
def savings_table(request):
    ...
    for income in income_by_month:
        ...
        total_expense = expense_by_month.filter(date__year=year, date__month=month).values('total_expense').first()
        total_expense = total_expense['total_expense'] if total_expense else 0
